...
async function addBot(bot) {
  await ensureTables();
  const res = await pool.query(
    `INSERT INTO bots (client_id, name, access_token, config) VALUES ($1,$2,$3,$4) RETURNING *;`,
    [bot.client_id, bot.name, bot.access_token, bot.config || {}]
  );
  return res.rows[0];
}

async function getBotsByClientId(clientId) {
  await ensureTables();
  const res = await pool.query(
    `SELECT * FROM bots WHERE client_id = $1 ORDER BY id ASC;`,
    [clientId]
  );
  return res.rows;
}
...
async function getBotByClientId(clientId) {
  await ensureTables();
  const res = await pool.query(
    `SELECT * FROM bots WHERE client_id = $1 LIMIT 1;`,
    [clientId]
  );
  return res.rows[0] || null;
}
...
async function getBotByPhoneNumberId(phone_number_id) {
  const client = await getClientByPhoneNumberId(phone_number_id);
  if (!client) return null;
  return await getBotByClientId(client.id);
}
...
module.exports = {
  ...
  updateBotState,
  updateBotAccessToken // ✅ NEW EXPORT
};
